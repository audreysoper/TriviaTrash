...
public class Answer extends Composite {
	...
	public Answer(Qbox p, boolean multipleChoice) {
		...
		if (multipleChoice) {
			...
			for (int i = 0; i < answerBoxes.length; i++) {
				...
				answerBoxes[i].setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true)); // grab vertical space
																							// because you'll match
																							// parent
				...
				correctSelector[i].setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false)); // don't want it
																									// grabbing extra
																									// horizontal
			...
			if (!answerFormatted.contains("^ ^ ^ ^") && answerFormatted.length() > 2) { // check if we ARE importing an
																						// answer
		...
		for (Text aBox : answerBoxes) {
			if (aBox.getText().length() > Board.charLimit - 1)
				aBox.setText(aBox.getText().substring(0, Board.charLimit - 1));
	...
	public String ansExport() throws Exception {
		
		if (multipleChoice) {
			if (correctAnswer.length() < 1) {
				MessageBox noAnswer = new MessageBox(this.getShell(), SWT.YES | SWT.NO);
				noAnswer.setMessage("It looks like question " + parent.getQNumber() + " from category "
						+ parent.getQobject(false).getCategory()
						+ " does not have an answer. Would you like to STOP and correct this?");
				if(noAnswer.open()==SWT.YES) throw new Exception("No answer found");
			}
